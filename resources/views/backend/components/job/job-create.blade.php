<div class="modal animated zoomIn" id="create-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Create Job</h5>
                </div>
                <div class="modal-body">
                    <form id="save-form">
                    <div class="container">
                        <div class="row">
                            <div class="col-12 p-1">
                                <label class="form-label">Job Title</label>
                                <input type="text" class="form-control" id="jobTitle">
                                <label class="form-label">Job Description</label>
                                <input type="text" class="form-control" id="jobDesc">

                            </div>
                        </div>
                    </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button id="modal-close" class="btn bg-gradient-primary" data-bs-dismiss="modal" aria-label="Close">Close</button>
                    <button onclick="Create()" id="save-btn" class="btn bg-gradient-success" >Save</button>
                </div>
            </div>
    </div>
</div>

<script>
async function Create()
{
    let title       = document.getElementById('jobTitle').value
    let jobDesc     = document.getElementById('jobDesc').value

    if(title.length == 0)
    {
        errorToast("Job title is required")
    }
    else if (jobDesc.length == 0)
    {
        errorToast("Job description is required")
    }
    else
    {
        document.getElementById('modal-close').click()

        let response = await axios.post('/create-job', {
            title:title,
            description:jobDesc,
        })

        if(response.status == 201)
        {
            successToast("Job created successfully");
            document.getElementById('save-form').reset()
            await getJobList()
        }
        else
        {
            errorToast("Job creation failed")
        }
    }
}
</script>
